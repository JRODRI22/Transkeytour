...
var require_machine = __commonJS({
  "../../node_modules/robot3/dist/machine.js"(exports) {
    ...
    var empty = () => ({});
    var identity = (a) => a;
    var callBoth = (par, fn, self, args2) => par.apply(self, args2) && fn.apply(self, args2);
    ...
    function stack(fns, def, caller) {
      return fns.reduce((par, fn) => {
        return function(...args2) {
          return caller(par, fn, this, args2);
        };
      }, def);
    }
    function fnType(fn) {
    ...
    function makeTransition(from, to, ...args2) {
      let guards = stack(filter(guardType, args2).map((t) => t.fn), truthy, callBoth);
      let reducers = stack(filter(reduceType, args2).map((t) => t.fn), identity, callForward);
      return create(this, {
    ...
    function state(...args2) {
      let transitions = filter(transitionType, args2);
      let immediates = filter(immediateType, args2);
      let desc = {
        final: valueEnumerable(args2.length === 0),
...
var require_realtime = __commonJS({
  "../../node_modules/@fal-ai/client/src/realtime.js"(exports) {
    ...
    function buildRealtimeUrl(app, { token, maxBuffering, path: path4 }) {
      ...
      const resolvedPath = (_a = (0, utils_1.resolveEndpointPath)(app, path4, "/realtime")) !== null && _a !== void 0 ? _a : "";
...
var require_sharp = __commonJS({
  "../../node_modules/sharp/lib/sharp.js"(exports, module) {
    ...
    for (path4 of paths) {
      try {
        sharp3 = __require(path4);
        break;
      } catch (err) {
        errors.push(err);
      }
    }
    if (sharp3 && path4.startsWith("@img/sharp-linux-x64") && !sharp3._isUsingX64V2()) {
...
var require_utility = __commonJS({
  "../../node_modules/sharp/lib/utility.js"(exports, module) {
    ...
    if (detectLibc.familySync() === detectLibc.GLIBC && !sharp3._isUsingJemalloc()) {
      ...
      sharp3.concurrency(__require("node:os").availableParallelism());
    }
...
__export(dist_exports, {
  assembleTileset: () => assembleTileset,
  buildPrompt: () => buildPrompt,
  compressSprite: () => compressSprite,
  ...
  llm: () => llm,
  llmJSON: () => llmJSON,
  processCharacterSheet: () => processCharacterSheet,
  processExistingImage: () => processExistingImage,
  processPropsSheet: () => processPropsSheet,
...
function buildPrompt(description, type) {
  switch (type) {
    case "walk":
...
async function generate(options) {
  ...
  const input = { prompt };
  if (refImage)
    input.image_url = refImage;
  const result = await import_client.fal.subscribe("fal-ai/nano-banana-pro", { input });
...
async function removeBgUrl(imageUrl) {
  const result = await import_client2.fal.subscribe("fal-ai/bria/background/remove", {
    input: {
      image_url: imageUrl
    }
  });
...
async function removeBg(imageBuffer) {
  ...
  const uploadedUrl = await import_client2.fal.storage.upload(file);
  const resultUrl = await removeBgUrl(uploadedUrl);
  const response = await fetch(resultUrl);
  if (!response.ok)
    throw new Error(`Failed to download bg-removed image: ${response.status}`);
  return Buffer.from(await response.arrayBuffer());
}
async function processCharacterSheet(input, options) {
  const cleaned = await floodFillRemoveBg(input);
  const trimmed = await (0, import_sharp.default)(cleaned).trim().png().toBuffer();
  const meta = await (0, import_sharp.default)(trimmed).metadata();
  ...
  for (let row = 0; row < GRID; row++) {
    for (let col = 0; col < GRID; col++) {
      ...
      let cell = await (0, import_sharp.default)(trimmed).extract({ left, top, width: extractW, height: extractH }).png().toBuffer();
      try {
        cell = await (0, import_sharp.default)(cell).trim().png().toBuffer();
      } catch {
      }
      const cellMeta = await (0, import_sharp.default)(cell).metadata();
      cells.push({
        buffer: cell,
  ...
  const heightsForScale = cells.filter((c) => c.height > 4 && (scaleRows ? scaleRows.includes(c.row) : true)).map((c) => c.height).sort((a, b) => a - b);
  ...
  for (const cell of cells) {
    if (cell.height < 4) {
      frames.push({
        buffer: await (0, import_sharp.default)({
    ...
    let resized = await (0, import_sharp.default)(cell.buffer).resize(scaledW, scaledH, { kernel: import_sharp.default.kernel.nearest }).png().toBuffer();
    let offsetX = 0;
    const offsetY = Math.max(0, FRAME_SIZE - scaledH);
    if (scaledW > FRAME_SIZE) {
      const cropLeft = Math.floor((scaledW - FRAME_SIZE) / 2);
      resized = await (0, import_sharp.default)(resized).extract({ left: cropLeft, top: 0, width: FRAME_SIZE, height: Math.min(scaledH, FRAME_SIZE) }).png().toBuffer();
    } else {
...
async function processPropsSheet(input) {
  const cleaned = await floodFillRemoveBg(input);
  const { data, info } = await (0, import_sharp.default)(cleaned).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const idx = y * w + x;
      if (pixels[idx * 4 + 3] === 0 || labels[idx] >= 0)
        continue;
  ...
  const significant = components.filter((c) => (c.maxX - c.minX) * (c.maxY - c.minY) > 500).sort((a, b) => {
  ...
  for (const c of significant) {
    ...
    const buffer = await (0, import_sharp.default)(cleaned).extract({ left: c.minX, top: c.minY, width: cw, height: ch }).png().toBuffer();
    pieces.push({ buffer, width: cw, height: ch });
  }
  return pieces;
}
async function processTexture(input, size = 32) {
  ...
  return (0, import_sharp.default)(input).extract({ left, top, width: cropSize, height: cropSize }).resize(size, size, { kernel: import_sharp.default.kernel.nearest }).png().toBuffer();
}
async function assembleTileset(tiles, size = 32, columns = 16) {
  const rows = Math.ceil(tiles.length / columns);
  const width = columns * size;
  const height = rows * size;
  const composites = await Promise.all(
    tiles.map(async (buf, i) => ({
      input: await (0, import_sharp.default)(buf).resize(size, size, { kernel: import_sharp.default.kernel.nearest }).png().toBuffer(),
      left: i % columns * size,
...
async function compressSprite(input, maxSize = 128) {
  const meta = await (0, import_sharp.default)(input).metadata();
  const w = meta.width, h = meta.height;
  if (w <= maxSize && h <= maxSize) {
    return (0, import_sharp.default)(input).png({ compressionLevel: 9, adaptiveFiltering: true }).toBuffer();
  }
  const scale = maxSize / Math.max(w, h);
  const newW = Math.max(1, Math.round(w * scale));
  const newH = Math.max(1, Math.round(h * scale));
  return (0, import_sharp.default)(input).resize(newW, newH, { kernel: import_sharp.default.kernel.nearest }).png({ compressionLevel: 9, adaptiveFiltering: true }).toBuffer();
}
async function floodFillRemoveBg(input) {
  const { data, info } = await (0, import_sharp.default)(input).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  const w = info.width, h = info.height;
  const pixels = Buffer.from(data);
  ...
  return (0, import_sharp.default)(pixels, { raw: { width: w, height: h, channels: 4 } }).png().toBuffer();
}
async function generateCharacter(options) {
  const sheetType = options.type ?? "walk";
  ...
  const processOpts = sheetType === "action" ? { scaleFromRows: [2, 3] } : void 0;
...
async function generateProps(options) {
  ...
  for (let i = 0; i < pieces.length; i++) {
    ...
    const meta = await (0, import_sharp2.default)(pieces[i].buffer).metadata();
...
async function generateObject(options) {
  ...
  const trimmed = await (0, import_sharp2.default)(imageBuffer).trim().toBuffer({ resolveWithObject: true });
  console.log("Compressing sprite...");
  const compressed = await compressSprite(trimmed.data);
  const compMeta = await (0, import_sharp2.default)(compressed).metadata();
  if (options.output) {
    mkdirSync(path.dirname(options.output), { recursive: true });
...
async function processExistingImage(imagePath, type, output, options) {
  let buffer = await (0, import_sharp2.default)(imagePath).png().toBuffer();
  if (!options?.skipBgRemoval) {
    console.log("Removing background...");
...
async function llm(options) {
  ...
  const result = await import_client3.fal.subscribe(endpoint, { input });
  const data = result.data;
  return { output: data.output };
}
async function llmJSON(options, retries = 2) {
...
function buildPlanPrompt(description, citizens) {
...
function buildVisionPlanPrompt(citizens) {
...
async function generateWorld(options) {
  ...
  const refImage = options.refImage ? await ensureUrl(options.refImage) : void 0;
  const spritesDir = path2.join(output, "world_assets", "props");
  const tilesDir = path2.join(output, "world_assets", "tiles");
  mkdirSync2(spritesDir, { recursive: true });
  mkdirSync2(tilesDir, { recursive: true });
  console.log("Planning world layout...");
  const plan = await llmJSON({
    prompt: refImage ? buildVisionPlanPrompt(citizens) : buildPlanPrompt(prompt, citizens),
    systemPrompt: PLAN_SYSTEM,
    imageUrl: refImage,
    model
  });
  console.log(`Plan: "${plan.name}" \u2014 ${plan.gridCols}x${plan.gridRows}, ${plan.textures.length} textures, ${plan.props.length} prop types`);
  writeFileSync2(path2.join(output, "plan.json"), JSON.stringify(plan, null, 2) + "\n");
  console.log(`Generating ${plan.textures.length} textures...`);
  await Promise.all(
    plan.textures.map(async (tex, i) => {
      const outPath = path2.join(tilesDir, `${tex.id}.png`);
  ...
  await Promise.all(
    plan.props.map(async (prop, i) => {
      const outPath = path2.join(spritesDir, `prop_${i}_${prop.id}.png`);
  ...
  const scenePath = path2.join(output, "world.json");
  writeFileSync2(scenePath, JSON.stringify(scene, null, 2) + "\n");
  console.log(`Scene saved: ${scenePath}`);
  return { scenePath, propsPaths, plan };
}
function buildScene(plan, citizenCount) {
  const props = [];
  const workAnchors = [];
  let pieceIndex = 0;
...
var import_client, import_client2, import_sharp, import_sharp2, import_client3, require2, BASE_STYLE, WALK_SHEET_STRUCTURE, WALK_SHEET_SUFFIX, ACTION_SHEET_STRUCTURE, PROPS_STRUCTURE, OBJECT_STRUCTURE, CHAIR_STRUCTURE, TEXTURE_STRUCTURE, FRAME_SIZE, GRID, DEFAULT_MODEL, PLAN_SYSTEM;
var init_dist = __esm({
  "../generate/dist/index.js"() {
    "use strict";
    import_client = __toESM(require_src(), 1);
    import_client2 = __toESM(require_src(), 1);
    import_sharp = __toESM(require_lib(), 1);
    import_sharp2 = __toESM(require_lib(), 1);
    import_client3 = __toESM(require_src(), 1);
    ...
    PLAN_SYSTEM = `You are a pixel art workspace designer for a top-down RPG-style office simulation game.
...
import { mkdirSync as mkdirSync3, writeFileSync as writeFileSync3, readFileSync as readFileSync2, existsSync as existsSync2, readdirSync } from "node:fs";
...
var AgentStore = class {
  ...
  sweepInterval = null;
  constructor(offlineTimeout = 12e4) {
    this.offlineTimeout = offlineTimeout;
  }
  start() {
    this.sweepInterval = setInterval(() => this.sweep(), 5e3);
  }
  stop() {
  ...
  heartbeat(data) {
    ...
    const agent = {
      ...
      task: data.task !== void 0 ? data.task : existing?.task ?? null,
...
// src/frontend.ts
function getFrontendHtml(wsPort) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
function render(agents) {
  ...
  agentsEl.innerHTML = agents.map(a =>
    '<div class="agent">' +
      ...
      '<span style="color:#555">' + a.state + (a.task ? ' \xB7 ' + a.task : '') + '</span>' +
    '</div>'
  ).join('');
}
</script>
...
var MiniverseServer = class {
  ...
  constructor(config = {}) {
    ...
    this.store = new AgentStore(config.offlineTimeout ?? 3e4);
  ...
  handleAction(agentId, action, senderWs) {
    ...
    if (actionType === "move" && typeof action.to === "string") {
      ...
      if (!this.channels.has(action.channel)) {
        this.channels.set(action.channel, /* @__PURE__ */ new Set());
      }
      this.channels.get(action.channel).add(agentId);
      if (senderWs)
        this.agentSockets.set(agentId, senderWs);
  ...
  queueInbox(agentId, from, message, channel) {
    if (!this.inbox.has(agentId))
  ...
  fireWebhook(agentId, from, message, channel) {
  ...
  routeMessage(fromId, action) {
    const msg = JSON.stringify({
      type: "message",
      from: fromId,
      message: action.message,
      channel: action.channel ?? void 0
    });
    if (typeof action.channel === "string") {
      ...
      for (const memberId of members) {
        ...
        if (ws && ws.readyState === WebSocket2.OPEN) {
          ...
          this.queueInbox(memberId, fromId, action.message, action.channel);
        }
      }
    } else if (action.to) {
      ...
      for (const targetId of targets) {
        ...
        if (ws && ws.readyState === WebSocket2.OPEN) {
          ...
          this.queueInbox(targetId, fromId, action.message);
        }
      }
    }
  ...
  // --- World editing ---
  getWorldPath(worldId) {
    const publicDir = this.publicDir ?? "./public";
    return path3.join(publicDir, "worlds", worldId, "world.json");
  }
  readWorld(worldId) {
    const worldPath = this.getWorldPath(worldId);
    if (!existsSync2(worldPath))
      return null;
    try {
      return JSON.parse(readFileSync2(worldPath, "utf-8"));
    } catch {
      return null;
    }
  }
  writeWorld(worldId, data) {
    const worldPath = this.getWorldPath(worldId);
    mkdirSync3(path3.dirname(worldPath), { recursive: true });
    writeFileSync3(worldPath, JSON.stringify(data, null, 2) + "\n");
    this.worldCache.delete(worldId);
  }
  async handleWorldAction(agentId, action) {
    const worldId = action.world ?? "cozy-startup";
    const actionType = action.type;
    try {
      if (actionType === "query_world") {
        const world = this.readWorld(worldId);
        if (!world) {
          this.queueInbox(agentId, "system", JSON.stringify({ error: "World not found", world: worldId }));
          return;
        }
        const summary = {
          type: "world_state",
          ...
          props: (world.props ?? []).map((p) => ({ id: p.id, x: p.x, y: p.y, w: p.w, h: p.h })),
      ...
      if (actionType === "paint_tile") {
        ...
        const world = this.readWorld(worldId);
        if (!world?.floor) {
          this.queueInbox(agentId, "system", JSON.stringify({ error: "World not found" }));
          return;
        }
      ...
      if (actionType === "place_prop") {
        ...
        if (!world.props)
          world.props = [];
        world.props = world.props.filter((p) => p.id !== id);
      ...
      if (actionType === "remove_prop") {
        const propId = action.id;
        if (!propId) {
          this.queueInbox(agentId, "system", JSON.stringify({ error: "remove_prop requires id" }));
          return;
        }
        const world = this.readWorld(worldId);
        if (!world) {
        ...
        world.props = (world.props ?? []).filter((p) => p.id !== propId);
        this.writeWorld(worldId, world);
      ...
      if (actionType === "generate_texture") {
        const { prompt, id } = action;
        if (!prompt || !id) {
          this.queueInbox(agentId, "system", JSON.stringify({ error: "generate_texture requires prompt, id" }));
          return;
        }
        this.queueInbox(agentId, "system", JSON.stringify({ status: "generating", action: "generate_texture", id }));
        let gen;
        try {
          gen = await Promise.resolve().then(() => (init_dist(), dist_exports));
        } catch {
          this.queueInbox(agentId, "system", JSON.stringify({ error: "@miniverse/generate not installed" }));
          return;
        }
        const publicDir = this.publicDir ?? "./public";
        const tilesDir = path3.join(publicDir, "worlds", worldId, "world_assets", "tiles");
        mkdirSync3(tilesDir, { recursive: true });
        const outPath = path3.join(tilesDir, `${id}.png`);
      ...
      if (actionType === "generate_prop") {
        const { prompt, id, w, h } = action;
        if (!prompt || !id) {
          this.queueInbox(agentId, "system", JSON.stringify({ error: "generate_prop requires prompt, id" }));
          return;
        }
        ...
        const publicDir = this.publicDir ?? "./public";
        const propsDir = path3.join(publicDir, "worlds", worldId, "world_assets", "props");
        mkdirSync3(propsDir, { recursive: true });
        const existing = existsSync2(propsDir) ? readdirSync(propsDir).filter((f) => f.startsWith("prop_")).length : 0;
        const filename = `prop_${existing}_${id}.png`;
        const outPath = path3.join(propsDir, filename);
  ...
  startKeepalive(agentId, agentName) {
  ...
  subagentKeepAlives = /* @__PURE__ */ new Map();
  handleClaudeCodeHook(data) {
    ...
    const agentId = data.agent ?? (shortSession ? `claude-${folder}-${shortSession}` : `claude-${folder}`);
    const agentName = data.name ?? (shortSession ? `Claude (${folder} #${shortSession})` : `Claude (${folder})`);
    const toolName = data.tool_name;
    const prompt = data.prompt;
    const subagentId = data.subagent_id;
    const subagentTask = data.subagent_task;
    switch (event) {
      case "SessionStart":
      ...
      case "Stop":
        ...
        break;
      case "SubagentStart": {
        const subId = subagentId ? `${agentId}-sub-${subagentId.slice(0, 6)}` : `${agentId}-sub-${Math.random().toString(36).slice(2, 8)}`;
        const subName = subagentTask ? `Claude (${truncate(subagentTask, 20)})` : `Claude (sub of ${folder})`;
        this.store.heartbeat({ agent: subId, name: subName, state: "working", task: subagentTask ?? "Running" });
        this.startKeepalive(subId, subName);
        if (!this.subagentKeepAlives.has(agentId))
          this.subagentKeepAlives.set(agentId, /* @__PURE__ */ new Set());
  ...
  getDefaultWorldId() {
    const publicDir = this.publicDir ?? "./public";
    const worldsDir = path3.join(publicDir, "worlds");
    if (!existsSync2(worldsDir))
      return null;
    try {
      const dirs = readdirSync(worldsDir).filter((d) => {
        return existsSync2(path3.join(worldsDir, d, "world.json"));
      });
      return dirs[0] ?? null;
    } catch {
      return null;
  ...
  loadWorldData(worldId) {
    ...
    const publicDir = this.publicDir ?? "./public";
    const worldPath = path3.join(publicDir, "worlds", worldId, "world.json");
    if (!existsSync2(worldPath))
      return null;
    try {
      const data = JSON.parse(readFileSync2(worldPath, "utf-8"));
  ...
  async handleHttp(req, res) {
    ...
    if (req.method === "POST" && url.pathname === "/api/heartbeat") {
      try {
        ...
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ ok: true, agent: { ...agent, lastSeen: void 0 } }));
      } catch (err) {
        res.writeHead(400, { "Content-Type": "application/json" });
    ...
    if (req.method === "GET" && url.pathname === "/api/inbox") {
      ...
      const messages = peek ? this.inbox.get(agent) ?? [] : this.drainInbox(agent);
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (req.method === "GET" && url.pathname === "/api/observe") {
      ...
      const snapshot = this.buildSnapshot(since ? parseInt(since, 10) : void 0);
  ...
  async handleGenerate(req, res) {
    ...
    try {
      ...
      if (data.falKey)
        process.env.FAL_KEY = data.falKey;
      ...
      const worldDir = worldId ? path3.join(publicDir, "worlds", worldId) : publicDir;
      let refImage;
      if (data.image) {
        const tmpPath = path3.join(tmpdir(), `miniverse_ref_${Date.now()}.png`);
        writeFileSync3(tmpPath, Buffer.from(data.image, "base64"));
        refImage = tmpPath;
      }
      let resultPath;
      let resultId;
      if (data.type === "props") {
        const spritesDir = path3.join(worldDir, "sprites");
        mkdirSync3(spritesDir, { recursive: true });
        const existing = readdirSync2(spritesDir).filter((f) => f.startsWith("prop_"));
        const nextIdx = existing.length;
        resultId = slug || "prop";
        const filename = `prop_${nextIdx}_${resultId}.png`;
        const outPath = path3.join(spritesDir, filename);
        ...
        const tilesDir = path3.join(worldDir, "tiles");
        mkdirSync3(tilesDir, { recursive: true });
        const outPath = path3.join(tilesDir, filename);
        ...
        const spritesDir = path3.join(publicDir, "sprites");
        mkdirSync3(spritesDir, { recursive: true });
        const outPath = path3.join(spritesDir, filename);
...
if (args[0] === "report") {
  ...
  const portIdx2 = args.indexOf("--port");
  const port2 = portIdx2 >= 0 ? args[portIdx2 + 1] : "4321";
  const body = JSON.stringify({ agent, state, task });
  await fetch(`http://localhost:${port2}/api/heartbeat`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body
  }).catch(() => {
...
var port = portIdx >= 0 ? parseInt(args[portIdx + 1], 10) : 4321;
var noBrowser = args.includes("--no-browser");
var server = new MiniverseServer({ port });
